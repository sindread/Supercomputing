...
void run_poisson(int numProcs, int rank, int numThreads, int n){
    ...
    */
    ...
    int start, end;
    start = rdispls[rank];
    end = rdispls[rank] + recvcounts[rank];
    ...
    */
    real **b = mk_2D_array(m, m, true);
    real **bt = mk_2D_array(m, m, true);
    ...
    */
    int nn = 4 * n;
    real *z = mk_1D_array(nn*numThreads, false);
    ...
    */
    #pragma omp parallel for num_threads(numThreads)
    for (size_t i = start; i < end; i++) {
        fst_(b[i], &n, (z+nn*omp_get_thread_num()), &nn);
    }
    ...
    #pragma omp parallel for num_threads(numThreads)
    for (size_t i = start; i < end; i++) {
        fstinv_(bt[i], &n, (z+nn*omp_get_thread_num()), &nn);
    }
    ...
    */
    #pragma omp parallel for num_threads(numThreads)
    for (size_t i = start; i < end; i++) {
        fst_(bt[i], &n, (z+nn*omp_get_thread_num()), &nn);
    }
    //transpose(b, bt, m);
